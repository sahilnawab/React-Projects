...
export default function TodosList() {
  ...
  const [todoMsg, setTodoMsg] = useState(todosList[0].title);
  ...
  return (
    <>
      ...
      <ul className="list-none">
        {todosList.map((todo) => (
          <li
            className="mt-4 flex justify-between items-center bg-slate-100 px-4 py-2 rounded"
            key={todo.id}
          >
            <input
              type="checkbox"
            ...
            <input
              type="text"
              className={`border m-2 outline-none w-full bg-transparent rounded-lg 
             ${todo.isUpdateble ? "border-black/10 px-2" : "border-transparent"}
              ...
              readOnly={!todo.isUpdateble}
            />

            <button
              onClick={() => dispatch(isUpdateble(todo.id))}
              className="text-white bg-yellow-100 border-0 py-1 px-4 focus:outline-none hover:bg-red-600 rounded text-md"
            >
            {todo.isUpdateble ? (
                <img className="w-6 h-6" src="https://img.icons8.com/ios/50/save.png" alt="save"/>
            ) : (
                <img className="w-6 h-6" src="https://img.icons8.com/ios/50/edit--v1.png" alt="edit--v1"/>
            )}
